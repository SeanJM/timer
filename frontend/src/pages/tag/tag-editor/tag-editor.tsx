...
function mapStateToProps(state: StoreState, props: TagEditorIncomingProps): TagEditorProps {
  const { categoryID, tagID } = props.params;
  const categoryResponse = state.todo.categories.find((a) => a.id === categoryID);
  const tagResponse = categoryResponse.tags.find((a) => a.id === tagID);
...
export const TagEditorConnect =
  withStore(TagEditorView, mapStateToProps)();